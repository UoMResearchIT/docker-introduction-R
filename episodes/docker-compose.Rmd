...
```bash
$ docker run -d --rm --name spuc_container -p 8321:8321 -v $PWD/print.config:/spuc/config/print.config -v spuc-volume:/spuc/output -v $PWD/stats.py:/spuc/plugins/stats.py -e EXPORT=true spuacv/spuc:latest --units iulu
```
...
| `-v $PWD/print.config:/spuc/config/print.config` | Mount the `print.config` file in the current directory to `/spuc/config/print.config` in the container |
| `-v spuc-volume:/spuc/output` | Persis the `/spuc/output` directory in the container to the `spuc-volume` volume |
...
services:
  spuc:
    ...
    ports:                          # Indicates that we are mapping ports
...
services:
  spuc:
    ...
    ports:
      - 8321:8321
    volumes:                                          # Indicates that we are mounting volumes
      - $PWD/print.config:/spuc/config/print.config   # Bind mounts the print.config file
```
...
services:
  spuc:
    ...
    ports:
      - 8321:8321
    volumes:
      - $PWD/print.config:/spuc/config/print.config
      - spuc-volume:/spuc/output              # Mounts the spuc-volume volume

volumes:                                      # Starts section for declaring volumes
  spuc-volume:                                # Declares the spuc-volume volume
```
...
services:
  spuc:
    ...
    environment:                      # Indicates that we are setting environment variables
...
services:
  spuc:
    ...
    volumes:
      - $PWD/print.config:/spuc/config/print.config
      - spuc-volume:/spuc/output
      - $PWD/stats.py:/spuc/plugins/stats.py        # Mounts the stats.py plugin
    environment:
      - EXPORT=true
    command: ["--units", "iulu"]
...
services:
  ...
  spucsvi:                            # Declare a new service named spucsvi
    ...
    container_name: spucsvi           # Name the container spucsvi
    ports:                            # 
...
* The image `spuacv/spucsvi:latest`
...
services:
  spuc:
    ...
    healthcheck:                                                  # Starts section for declaring healthchecks
      test: ["CMD", "curl", "--fail", "http://spuc:8321/export"]  # Specifies the healthcheck command
      interval: 5s                                                # Specifies the interval between healthchecks
      timeout: 10s                                                # Specifies the timeout for the healthcheck
      retries: 3                                                  # Specifies the number of retries before failing
...
services:
  spuc:
    ...
    environment:
      - EXPORT=true
    command: ["--units", "iulu"]
    entrypoint: ["sh", "-c", "sleep 10 && python /spuc/spuc.py"]   # Adds a sleep command to the entrypoint to simulate a slow start
    healthcheck:
